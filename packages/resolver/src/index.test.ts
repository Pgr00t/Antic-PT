...
describe("AnticipationResolver.applyPatch", () => {
  it("applies replace op to top-level field", () => {
    ...
    const result = AnticipationResolver.applyPatch(
      { cpuUsage: 0.73, memoryUsage: 0.61 },
      ops,
    );
  ...
  it("emits speculative then confirm when X-Antic-State is confirmed", async () => {
    ...
    const mockFetch = (_url: string, _opts: any) =>
      Promise.resolve({
        ok: true,
        headers: {
          get: (h: string) => (h === "X-Antic-State" ? "confirmed" : null),
        },
    ...
    resolver
      .on("speculative", (data) => {
        capturedEvents.push("speculative");
        capturedData = data;
      })
